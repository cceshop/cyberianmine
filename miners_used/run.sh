...
## GET_DATA
readonly url_machines_used="https://my.cyberianmine.de/marketplace"
...
declare -a l_miners_rawdata=(`grep -A 10 'class="card-title">' ${f_shop} \
	| grep -iE '(card-title)|(Price, EUR)|(Revenue)|(Hosting)|(ROI)'`)
...
do
  ...
  miner_price=$(echo ${miner} | cut -f 2 -d '#')
  miner_revenue=$(echo ${miner} | cut -f 3 -d '#')
  miner_hosting=$(echo ${miner} | cut -f 4 -d '#')
  miner_roi=$(echo ${miner} | cut -f 5 -d '#')
