...
if [[ ${C_CM_OWNED} -lt ${#l_miners_data[*]} ]]; then
  n_sold=$(echo "${C_CM_OWNED} - ${#l_miners_data[*]}" | bc)
  if [[ ${n_sold} -gt 1 ]]; then
    echo "${n_sold} machines were sold."
  else
    echo "${n_sold} machines was sold."
  fi
else
  echo "No machine was sold."
fi
